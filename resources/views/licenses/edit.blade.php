...
@section('inputFields')
        ...
        <div class="col-md-7 col-sm-12">
            <div class="col-md-12" style="padding-left:0px">
                <input class="form-control" type="text" name="seats" id="seats"
                    value="{{ old('seats', $item->seats) }}" minlength="1" required style="width: 97px;">
            </div>
        </div>
        {!! $errors->first(
            'seats',
    ...
    @can('viewKeys', $item)
        <div class="form-group {{ $errors->has('serial') ? ' has-error' : '' }}">
            ...
            <div class="col-md-7">
                <textarea class="form-control" type="text" name="serial"
                    id="serial"{{ Helper::checkIfRequired($item, 'serial') ? ' required' : '' }}>{{ old('serial', $item->serial) }}</textarea>
                {!! $errors->first(
                    'serial',
    ...
    <div class="form-group {{ $errors->has('license_email') ? ' has-error' : '' }}">
        ...
        <div class="col-md-7">
            <input class="form-control" type="email" name="license_email" id="license_email"
                value="{{ old('license_email', $item->license_email) }}" />
            {!! $errors->first(
                'license_email',
                '<span class="alert-msg" aria-hidden="true"><i class="fas fa-times" aria-hidden="true"></i> :message</span>',
            ) !!}
    ...
    <div class="form-group {{ $errors->has('reassignable') ? ' has-error' : '' }}">
        ...
        <div class="col-md-7">
            <label class="form-control">
                {{ Form::Checkbox('reassignable', '1', old('reassignable', $item->id ? $item->reassignable : '1'), ['aria-label' => 'reassignable']) }}
                {{ trans('general.yes') }}
            </label>
        </div>
    </div>
    ...
    <div class="form-group {{ $errors->has('termination_date') ? ' has-error' : '' }}">
        <label for="termination_date"
            class="col-md-3 control-label">{{ trans('admin/licenses/form.termination_date') }}</label>
    ...
    <div class="form-group {{ $errors->has('purchase_order') ? ' has-error' : '' }}">
        <label for="purchase_order"
            class="col-md-3 control-label">{{ trans('admin/licenses/form.purchase_order') }}</label>
        <div class="col-md-3">
            <input class="form-control" type="text" name="purchase_order" id="purchase_order"
                value="{{ old('purchase_order', $item->purchase_order) }}" maxlength="191" />
            {!! $errors->first(
                'purchase_order',
                '<span class="alert-msg" aria-hidden="true"><i class="fas fa-times" aria-hidden="true"></i> :message</span>',
            ) !!}
    ...
    <div class="form-group {{ $errors->has('maintained') ? ' has-error' : '' }}">
        ...
        <div class="col-md-7">
            <label class="form-control">
                {{ Form::Checkbox('maintained', '1', old('maintained', $item->maintained), ['aria-label' => 'maintained']) }}
                {{ trans('general.yes') }}
            </label>
        </div>
    </div>
