                    ...
                    @can('view', \App\Models\Component::class)
                        <li aria-hidden="true"{!! Request::is('components*') ? ' class="active"' : '' !!}>
                            <a href="{{ route('components.index') }}"
                                ...
                                data-tooltip="true" data-placement="bottom"
                                data-title="{{ trans('general.components') }}">
                            ...
                            <li class="dropdown tasks-menu">
                                <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                                    ...
                                    @if (count($alert_items) || count($deprecations))
                                        <span
                                            class="label label-danger">{{ count($alert_items) + count($deprecations) }}</span>
                                    @endif
                                </a>
                                <ul class="dropdown-menu">
                                    @if ($deprecations)
                                        ...
                                        @endforeach
                                    @endif
                                    <li class="header">{{ trans_choice('general.quantity_minimum', count($alert_items)) }}
                                    </li>
                                    <li>
                    ...
                    <li{!! Request::query('status') == 'Deployed' ? ' class="active"' : '' !!}>
                        ...
                        <li{!! Request::query('status') == 'RTD' ? ' class="active"' : '' !!}>
                            ...
                            <li{!! Request::query('status') == 'Pending' ? ' class="active"' : '' !!}><a href="{{ url('hardware?status=Pending') }}">
                                ...
                                <li{!! Request::query('status') == 'Undeployable' ? ' class="active"' : '' !!}><a href="{{ url('hardware?status=Undeployable') }}">
                                    ...
                                    <li{!! Request::query('status') == 'byod' ? ' class="active"' : '' !!}><a href="{{ url('hardware?status=byod') }}">
                                            <x-icon type="x" class="text-red fa-fw" />
                                            {{ trans('general.byod') }}
                                            <span
                                        ...
                                        <li{!! Request::query('status') == 'Archived' ? ' class="active"' : '' !!}><a href="{{ url('hardware?status=Archived') }}">
                                                <x-icon type="x" class="text-red fa-fw" />
    ...
    <footer class="main-footer hidden-print" style="display:grid;flex-direction:column;">
        ...
        <div class="1hidden-xs pull-left">
            <div class="pull-left">
                ...
                made with <x-icon type="heart" style="color: #a94442; font-size: 10px" />
                <span class="sr-only">love</span> by <a href="https://bsky.app/profile/snipeitapp.com"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (companySelect) {
            ...
            $(companySelect).select2({
                ajax: {
                    url: '/api/v1/companies/selectlist?page=1', // API endpoint for companies
                    dataType: 'json',
                    headers: {
                ...
                if (selectedCompanyId && departmentSelect) {
                    // Clear existing department options
                    departmentSelect.innerHTML = '';

                    // Fetch associated departments
                    fetch(`/api/v1/departments/?company_id=${selectedCompanyId}`, {
                            method: 'GET',
                            headers: {
