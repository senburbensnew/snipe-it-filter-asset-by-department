    ...
    <div id="{{ $fieldname }}" class="form-group{{ $errors->has($fieldname) ? ' has-error' : '' }}">
        ...
        <div class="col-md-8">
            <select class="js-data-ajax" data-endpoint="companies"
                ...
                style="width: 100%"
                id="company_select"{{ isset($multiple) && $multiple == 'true' ? " multiple='multiple'" : '' }}>
                @if ($company_id = old($fieldname, isset($item) ? $item->{$fieldname} : ''))
        ...
        {!! $errors->first(
            $fieldname,
            '<div class="col-md-8 col-md-offset-3"><span class="alert-msg"><i class="fas fa-times" aria-hidden="true"></i> :message</span></div>',
        ) !!}
